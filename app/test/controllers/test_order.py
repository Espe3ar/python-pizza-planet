...
def __order(ingredients: list, size: dict, client_data: dict, beverages:list):
    ingredients_ids = [ingredient.get('_id') for ingredient in ingredients]
    beverages_ids = [beverage.get('_id') for beverage in beverages]
...
def test_create(app, ingredients, size, client_data, beverages):
    ...
    for param, value in order.items():
        ...
        beverages_in_detail = set(item['beverage']['_id'] for item in created_order['beverages'])
